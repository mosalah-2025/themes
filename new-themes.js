...
(function() {
  ...
  const THEMES = [
    {id:'nature',label:'Nature'},{id:'hype',label:'Hype'},{id:'elite',label:'Elite'},
    {id:'phoenix',label:'Phoenix'},{id:'pearl',label:'Pearl'},{id:'blast',label:'Blast'},{id:'volta',label:'Volta'},
  ];
  const COLORS = [
    {id:'dark',label:'Dark Mode',dot:'dot-dark'},{id:'light',label:'Light Mode',dot:'dot-light'},
  ];
  const CATS = [
    {id:'fashion',label:'Fashion & Clothing'},{id:'shoes',label:'Shoes & Bags'},
    {id:'watches',label:'Watches & Eyewear'},{id:'perfumes',label:'Perfumes'},
    {id:'cosmetics',label:'Cosmetics'},{id:'health',label:'Health & Fitness'},
    {id:'electronics',label:'Electronics'},{id:'furniture',label:'Furniture & Decor'},
    {id:'kids',label:'Baby & Kids'},{id:'gifts',label:'Gifts & Handmade'},
    {id:'pets',label:'Pet Supplies'},{id:'garden',label:'Garden & Outdoor'},
    {id:'books',label:'Books'},{id:'kitchen',label:'Home & Kitchen'},
    {id:'automotive',label:'Automotive & Accessories'},{id:'digital',label:'Digital Products'},
    {id:'tools',label:'Tools & Equipment'},{id:'general',label:'General Store'},
  ];

  function buildFilterHTML(prefix) {
    return makeSection(prefix,'t','Theme',THEMES,false)
  ...
  function makeSection(prefix,key,title,items,isColor) {
    let html = `<div class="filter-section"><div class="filter-section-header" onclick="toggleSection(this)"><span class="label">${title}</span><span class="arrow"></span></div><div class="filter-body open"><div class="filter-body-inner">`;
    items.forEach(item => {
      html += `<div class="filter-option"><input type="checkbox" id="${prefix}${key}-${item.id}" onchange="applyFilters()" /><span class="box" onclick="document.getElementById('${prefix}${key}-${item.id}').click()"></span>${isColor?`<span class="color-dot ${item.dot}"></span>`:''}<label for="${prefix}${key}-${item.id}">${item.label}</label></div>`;
    });
  ...
  function syncPair(sId,pId) {
    ...
    if (!s||!p) return;
    if (document.activeElement===s) p.checked=s.checked;
    else if (document.activeElement===p) s.checked=p.checked;
  }
  ...
  function getLabelById(list,id) {
    return (list.find(i=>i.id===id)||{label:id}).label;
  }
  ...
  window.handleSearch = function(inputEl) {
    ...
    if (ds) ds.value = inputEl.value;
    if (ms) ms.value = inputEl.value;
    applyFilters();
  };
  ...
  window.removeFilter = function(type,id) {
    if (type==='search') { searchQuery=''; window.resetAllFilters(); return; }
    const s = document.getElementById('s-'+type+'-'+id);
    const p = document.getElementById('p-'+type+'-'+id);
  ...
  function renderActiveFilters(aT,aC,aCa) {
    ...
    if (!box) return;
    let chips=[];
    aT.forEach(id=>chips.push(`<span class="filter-chip">${escapeHtml(getLabelById(THEMES,id))}<button type="button" onclick="removeFilter('t','${id}')">×</button></span>`));
    aC.forEach(id=>chips.push(`<span class="filter-chip">${escapeHtml(getLabelById(COLORS,id))}<button type="button" onclick="removeFilter('col','${id}')">×</button></span>`));
    aCa.forEach(id=>chips.push(`<span class="filter-chip">${escapeHtml(getLabelById(CATS,id))}<button type="button" onclick="removeFilter('c','${id}')">×</button></span>`));
    if (searchQuery) chips.push(`<span class="filter-chip">Search: ${escapeHtml(searchQuery)}<button type="button" onclick="removeFilter('search')">×</button></span>`);
    if (!chips.length) { box.classList.remove('visible'); box.innerHTML=''; return; }
    box.classList.add('visible');
    box.innerHTML=`<span class="active-filters-label">FILTERED BY:</span>${chips.join('')}<button type="button" class="clear-filters-btn" onclick="resetAllFilters()">Clear</button>`;
  }
  ...
  window.applyFilters = function() {
    ...
    const aT=getActive('t',THEMES), aC=getActive('col',COLORS), aCa=getActive('c',CATS);
    renderActiveFilters(aT,aC,aCa);
    filteredCards=ALL_CARDS.filter(card=>{
      return (aT.length===0||aT.includes(card.theme))
          && (aC.length===0||aC.includes(card.color))
  ...
  function renderPage() {
    ...
    if (countEl) countEl.textContent=total+' theme'+(total!==1?'s':'');
    grid.innerHTML = slice.length===0
      ? `<div style="grid-column:1/-1;padding:40px 0;text-align:center;color:#777;">No themes found.</div>`
      : slice.map(card=>`<div class="card"><div class="card-img-wrap"><a href="${card.url}" target="_blank"><img class="card-img" src="${card.img}" alt="${card.name}" loading="lazy"/></a></div><div class="card-info"><span class="card-name">${card.name}</span><span class="badge-new">New</span></div></div>`).join('');
  ...
  function renderPagination(totalPages) {
    ...
    let html=`<button class="page-btn arrow-btn" onclick="goPage(${currentPage-1})" ${currentPage===1?'disabled':''}>&#8249;</button>`;
    for(let i=1;i<=totalPages;i++){
      if(i===1||i===totalPages||(i>=currentPage-1&&i<=currentPage+1)) html+=`<button class="page-btn ${i===currentPage?'active':''}" onclick="goPage(${i})">${i}</button>`;
      else if(i===currentPage-2||i===currentPage+2) html+=`<span style="padding:0 4px;color:#999">…</span>`;
    }
    html+=`<button class="page-btn arrow-btn" onclick="goPage(${currentPage+1})" ${currentPage===totalPages?'disabled':''}>&#8250;</button>`;
    pag.innerHTML=html;
  }
  ...
  window.openPopup  = function(){ const p=document.getElementById('popup'); if(p){p.classList.add('open');document.body.style.overflow='hidden';} };
  window.closePopup = function(){ const p=document.getElementById('popup'); if(p){p.classList.remove('open');document.body.style.overflow='';} };
  window.handleOverlayClick = function(e){ if(e.target===document.getElementById('popup')) window.closePopup(); };
  ...
  function tryInit() {
    ...
    const popup   = document.getElementById('popup-filters');
    if (!grid || !sidebar || !popup) return;

    initialized = true;

    sidebar.innerHTML = buildFilterHTML('s-');
    popup.innerHTML   = buildFilterHTML('p-');

    filteredCards = [...ALL_CARDS];
    renderPage();
  }

  // بيحاول كل 100ms لحد ما يلاقي العناصر — بيوقف لوحده بعد ما ينجح
  const interval = setInterval(function() {
    tryInit();
    if (initialized) clearInterval(interval);
  }, 100);

  // وقف بعد 10 ثواني على الأكتر عشان ما يفضلش شغال
